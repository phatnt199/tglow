...
/**
 * A rule and a prompt: two rows always, three while replying. Letting the
 * prompt wrap would take a row from the panes above without telling them, and
 * a column given more rows of children than it has shrinks them until they
 * overdraw each other -- see message-view.tsx's own rail for the same
 * invariant. App accounts for the extra row the same way it already does for
 * the which-key overlay's variable height, so this growing by one row here
 * must stay in lockstep with app.tsx's own chromeHeight calculation.
 */
export const Composer = (props: IComposerProps) => {
  const { text, mode, focused, tokens, width, replyingTo } = props;
